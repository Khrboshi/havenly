...
export default function Progress() {
  ...
  useEffect(() => {
    ...
    const totalWords = reflections.reduce((sum, r) => sum + r.text.split(" ").length, 0);
    const avgWords = (totalWords / reflections.length).toFixed(0);

    const sorted = [...reflections].sort((a, b) => new Date(a.date) - new Date(b.date));
    setStats({
        ...
        {stats.total === 0 ? (
          ...
          >
            <div className="bg-white/90 backdrop-blur border border-slate-200 shadow-sm rounded-2xl p-6">
              <h2 className="text-xl font-semibold text-slate-700 mb-2">Total Reflections</h2>
              <p className="text-4xl font-bold text-blue-600">{stats.total}</p>
            </div>

            <div className="bg-white/90 backdrop-blur border border-slate-200 shadow-sm rounded-2xl p-6">
              <h2 className="text-xl font-semibold text-slate-700 mb-2">This Month</h2>
              <p className="text-4xl font-bold text-blue-600">{stats.thisMonth}</p>
            </div>

            <div className="bg-white/90 backdrop-blur border border-slate-200 shadow-sm rounded-2xl p-6">
              <h2 className="text-xl font-semibold text-slate-700 mb-2">Average Words</h2>
              <p className="text-4xl font-bold text-blue-600">{stats.avgWords}</p>
            </div>

            <div className="bg-white/90 backdrop-blur border border-slate-200 shadow-sm rounded-2xl p-6">
              <h2 className="text-xl font-semibold text-slate-700 mb-2">Activity Period</h2>
